...
import { Plus, Pencil, Trash2, Search, Loader2, TrendingDown, X, Filter } from 'lucide-react';
...
export default function ExpensePage() {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().slice(0, 10), purpose: '', amount: '', note: '' });
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      const matchSearch = r.purpose.toLowerCase().includes(q) || (r.note ?? '').toLowerCase().includes(q);
  ...
  const openEdit = (r: Expense) => {
    setEditing(r);
    setForm({ date: r.date, purpose: r.purpose, amount: String(r.amount), note: r.note ?? '' });
    setError(null);
    setModalOpen(true);
  };
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (!form.date || !form.purpose || !form.amount) {
      setError('Tanggal, keperluan, dan nominal wajib diisi');
      return;
    }
    setSaving(true);
                  ...
                  <tr key={r.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-2">
                        {user && <button onClick={() => openEdit(r)} className="p-2 text-primary-600 hover:bg-primary-50 rounded-lg transition">
                          <Pencil className="w-4 h-4" />
                        </button>}
                        {user && <button onClick={() => setDeleteId(r.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Pengeluaran' : 'Tambah Pengeluaran'}>
        <form onSubmit={save} className="space-y-4">
          ...
          <div>
            ...
            <input value={form.purpose} onChange={(e) => setForm({ ...form, purpose: e.target.value })} placeholder="cth: Beli alat tulis" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          <div>
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg shadow-sm transition disabled:opacity-60">
